...
plot(x=RainScotland$Elevation, y=RainScotland$Rainfall,
    ...
    xlim=c(0,600), ylim=c(0,2500), yaxs="i", xaxs="i")
```
...
text(x=RainScotland$Elevation[18]+25,y=RainScotland$Rainfall[18]+100,labels = "e_18", col="orange")
...
In order to know how significant are the estimated parameters and the explained variance, we need a hypothesis related to how good the line estimated with our sample points is to represent the true relationship between rainfall and elevation.
...
alpha<-1000
beta<-2
abline(a=alpha,b=beta, col="goldenrod")
text(x=280,y=1900,expression(paste("True line ", Y == alpha + beta * X)), col="goldenrod")
```
...
alpha<-1000
beta<-2
epsilon<-rnorm(n=nrow(RainScotland), mean = 0,sd = 350)
abline(a=alpha,b=beta, col="goldenrod")
text(x=170,y=2250,expression(paste("True line + normal errors:  ", Y[i] == alpha + beta * X[i] + epsilon[i])), col="goldenrod")
points(x=RainScotland$Elevation, y=alpha+beta*RainScotland$Elevation+epsilon, col="goldenrod")
```
...
epsilon<-rnorm(n=nrow(RainScotland), mean = 0,sd = 350)
abline(a=alpha,b=beta, col="goldenrod")
text(x=170,y=2300,expression(paste("error terms:  ", epsilon[i])), col="goldenrod")
text(x=170,y=2100,expression(paste("residuals:  ", e[i])), col="blue")
text(x=330,y=1350,expression(epsilon[i]), col="goldenrod")
text(x=310,y=1350,expression(e[i]), col="blue")
points(x=RainScotland$Elevation, y=alpha+beta*RainScotland$Elevation+epsilon, col="goldenrod")
abline(a=OLS_a,b=OLS_b, col="blue")
segments(x0=RainScotland$Elevation[19]-2,y0=RainScotland$Rainfall[19],
         x1=RainScotland$Elevation[19]-2, y1=l_a+l_b*RainScotland$Elevation[19], col="blue",lwd=2)
segments(x0=RainScotland$Elevation[19]+2,
         y0=alpha+beta*RainScotland$Elevation[19]+epsilon[19],
         x1=RainScotland$Elevation[19]+2,
         y1=alpha+beta*RainScotland$Elevation[19], col="goldenrod",lwd=2)
```
