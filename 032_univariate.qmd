...
$$\dfrac {\sum_{i=1}^{n} (x_i - \bar{x})^4} {n s^4}$$
However it is adjusted with some correction for small sample sizes and for comparison to a normal distribution for which the result would be 3. See the help for how it is computed by default
...
Using this implementation, a Kurtosis close to 0 then indicates a distribution similar in shape to a normal (bell-shaped) distribution. 
A **positive kurtosis indicates a more peaked** distribution, also named **leptokurtic**.
